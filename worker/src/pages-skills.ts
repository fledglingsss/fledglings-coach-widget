...
@keyframes fl-drop{0%{opacity:0;transform:translateY(-14px) rotate(0deg)}10%{opacity:1}
  100%{opacity:0;transform:translateY(360px) rotate(560deg)}}
@keyframes fl-tw{0%,100%{transform:scale(.65);opacity:.35}50%{transform:scale(1.15);opacity:1}}
.page{padding:40px 20px;display:flex;justify-content:center}
.shell{width:100%;max-width:1160px;border-radius:28px;overflow:hidden;background:#fff;position:relative;
  box-shadow:0 30px 70px -30px rgba(5,37,60,.45);border:1px solid rgba(5,37,60,.07)}
.confetti{position:absolute;top:0;left:0;right:0;height:0;pointer-events:none;z-index:6}
.confetti i{position:absolute;top:-14px;width:9px;height:14px;border-radius:2px;opacity:0;
  animation:fl-drop 3s ease-in both}
.brand{display:flex;align-items:center;gap:11px;font-size:20px;font-weight:800;color:#05253C;letter-spacing:.2px}
.brand .dots{display:flex;gap:5px}
.brand .dots i{width:11px;height:11px;border-radius:50%;display:block}
.brand .dots i:nth-child(1){background:#D9452B}
.brand .dots i:nth-child(2){background:#ED9249}
.brand .dots i:nth-child(3){background:#13507F}
...
.avatar{width:84px;height:84px;border-radius:24px;background:linear-gradient(135deg,#A66633,#B93A22);
...
.grade{background:linear-gradient(135deg,#A66633,#B93A22);color:#fff;padding:18px 28px;border-radius:18px;
  text-align:center;position:relative;overflow:hidden}
.grade .t{font-size:12px;font-weight:500;opacity:.9}
.grade .g{font-size:28px;font-weight:700}
.grade .sp{position:absolute;font-size:13px;animation:fl-tw 2.2s ease-in-out infinite;pointer-events:none}
...
.career .cs{font-size:12.5px;color:#5C6A76;margin-top:2px}
.career .cs b{color:#1A7649}
...
.stat.st2{background:#FCEBE4}.stat.st2 .v{color:#B93A22}.stat.st2 .h{color:#B93A22}
.stat.st3{background:#FBF0E2}.stat.st3 .v{color:#9A5812}.stat.st3 .h{color:#9A5812}
.stat.st4{background:#E6EBEF}.stat.st4 .v{color:#05253C}
...
.sect-h .c{color:#B93A22;font-size:13.5px;font-weight:600}
...
.skill .r b{color:#B93A22;font-weight:700}
...
.mod .p{color:#B93A22;font-weight:700;font-size:14px;justify-self:end}
...
export function renderSkillsPassport(
  ...
  /* Rotate through the brand palette so the page feels lively rather
   * than single-accent. */
  const BAR_GRADS = [
    "linear-gradient(90deg,#ED9249,#D9452B)",
    "linear-gradient(90deg,#4A7FAC,#13507F)",
    "linear-gradient(90deg,#F3B36B,#ED9249)",
    "linear-gradient(90deg,#13507F,#05253C)",
  ];
  const CONFETTI_COLOURS = ["#D9452B", "#ED9249", "#13507F", "#05253C", "#F3B36B"];
  const confettiHtml =
    "<div class='confetti' aria-hidden='true'>" +
    Array.from({ length: 18 }, (_, i) => {
      const left = (i * 53 + 7) % 100;
      const delay = ((i * 37) % 22) / 10;
      const colour = CONFETTI_COLOURS[i % CONFETTI_COLOURS.length];
      const tilt = (i * 47) % 70 - 35;
      return `<i style='left:${left}%;background:${colour};animation-delay:${delay.toFixed(1)}s;transform:rotate(${tilt}deg)'></i>`;
    }).join("") +
    "</div>";
  ...
  const skillsHtml = model.skills
    .map(
      (sk, i) =>
        ...
        `<i style='width:${sk.percent}%;background:${BAR_GRADS[i % BAR_GRADS.length]};animation-delay:${(0.1 + i * 0.1).toFixed(1)}s'></i></div></div>`,
    )
    .join("");
  ...
  const body =
    ...
    confettiHtml +
    "<div class='topbar'>" +
    "<div class='brand'><span class='dots' aria-hidden='true'><i></i><i></i><i></i></span>Skills Passport</div>" +
    "<div class='nav'>" +
    ...
    "<div class='scorewrap'>" +
    `<div class='ring' style='background:conic-gradient(#ED9249 0deg,#D9452B ${Math.round(deg * 0.6)}deg,#13507F ${deg}deg,#ECE7E6 ${deg}deg)' role='img' aria-label='Score ${model.score.percent} percent'>` +
    `<div class='in'><div class='pc'>${model.score.percent}%</div><div class='lb'>SCORE</div></div></div>` +
    "<div class='grade'><span class='sp'>✦</span><span class='sp'>✦</span>" +
    `<div class='t'>Overall grade</div><div class='g'>${esc(model.score.grade)}</div></div>` +
    "</div></div>" +
    /* stat cards */
    "<div class='cards'>" +
    `<div class='stat st1'><div class='h'>${ICONS.clock}<span>Time invested</span></div>` +
    `<div class='v'>${esc(formatMinutes(s.totalMinutes))}</div><div class='s'>avg ${esc(formatMinutes(s.avgMinutesPerModule))} / module</div></div>` +
    `<div class='stat st2'><div class='h'>${ICONS.flame}<span>Streak</span></div>` +
